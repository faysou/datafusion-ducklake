    ...
    assert_eq!(
        ...
        vec![
            (
                ...
                format!(
                    "created_schema:\"public\",created_table:\"events\",\
                     inserted_into_table:{}",
                    first.table_id
                ),
                None,
...
#[tokio::test(flavor = "multi_thread")]
#[cfg_attr(all(feature = "skip-tests-with-docker", target_os = "macos"), ignore)]
async fn postgres_restore_references_source_files_with_new_ids() {
    use datafusion_ducklake::{
        DeleteFileInfo,
    ...
    assert_eq!(
        (
            live_data[0].try_get::<String, _>("path").unwrap(),
    ...
    assert_eq!(
        (
            ...
            live_delete.try_get::<String, _>("path").unwrap(),
    ...
    assert_eq!(
        scheduled,
        vec![
            "cat_1/public/events/first-deletes.parquet",
            "cat_1/public/events/first.parquet",
            "cat_1/public/events/second.parquet",
        ],
    );

    let store: Arc<dyn ObjectStore> = Arc::new(LocalFileSystem::new());
    let cleanup =
        cleanup_old_files_in_catalog(&manager, "restore", store, CleanupCriteria::All, true)
            .await
            .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            data_path
                .join("cat_1/public/events/second.parquet")
                .to_string_lossy()
                .to_string()
        ],
    );
    ...
    assert_eq!(
        ...
        vec![
            ...
            (
                ...
                format!(
                    "created_schema:\"ma\"\"in\",created_table:\"ev\"\"ents\",\
                     inserted_into_table:{}",
                    setup.table_id,
                ),
            ),
            ...
            (reset_snapshot, format!("altered_table:{}", setup.table_id),),
        ],
    );
}
...
async fn postgres_partitioned_write_honors_expected_snapshot() {
    ...
    assert!(
        ...
        "unexpected partitioned-write error: {error}",
    );
}
