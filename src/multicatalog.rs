...
pub async fn initialize_multicatalog_schema(pool: &PgPool) -> Result<()> {
    ...
    // change. Idempotent (no-op once composite). This is the multicatalog
...
impl MulticatalogManager {
    ...
    /// Read the `data_path` for one catalog, falling back to legacy global metadata.
    ...
    pub(crate) async fn clear_dropped_data_path(&self, data_path: &str) -> Result<()> {
        sqlx::query("DELETE FROM ducklake_dropped_data_path WHERE data_path = $1")
            .bind(data_path)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    /// matches `data_path`. Shared-root cleanup must preserve peer catalogs.
    pub(crate) async fn list_referenced_paths_in_data_path(
        &self,
        data_path: &str,
