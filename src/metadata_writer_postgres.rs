...
async fn finalize_snapshot(
    ...
    // column_id -> (column_order, nullable) for the surgical update. The
    ...
    let mut current_by_id: std::collections::HashMap<i64, (i64, bool)> =
    ...
    for row in &existing_column_rows {
        ...
        current_by_id.insert(id, (order, nullable));
    }
                ...
                "SELECT EXISTS(
                    ...
                    WHERE table_id = $1
                      AND (begin_snapshot > $2
                           OR (end_snapshot IS NOT NULL AND end_snapshot > $2))
                 )",
            )
            .bind(table_id)
            .bind(source_snapshot_id)
