...
use crate::metadata_writer::{
    ...
    MetadataWriter, RestoreResult, SnapshotCommitMetadata, WriteMode, WriteSetupResult,
    columns_differ, restored_table_data_changes, table_write_changes, validate_delete_entries,
    validate_name,
};
...
mod tests {
    ...
    async fn restore_references_source_files_with_new_ids_and_cleanup_keeps_live_paths() {
        ...
        let commit_metadata = SnapshotCommitMetadata::new()
            .with_author("restore-test")
            .with_message("restore source state")
            .with_extra_info("restore-id=7");
        let restored = writer
        ...
        assert_eq!(
            ...
            (
                ...
                Some("restore source state".to_string()),
                Some("restore-id=7".to_string()),
            ),
        );
